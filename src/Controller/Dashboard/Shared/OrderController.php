...
class OrderController extends Controller {
    ...
    public function checkout(Request $request, TranslatorInterface $translator, AppServices $services, RouterInterface $router) {
        ...
        if ($form->isSubmitted()) {
            ...
            if ($form->isValid()) {
                ...
                if ($request->request->get('payment_gateway') == "mercadopago_checkout") {
                    //dump($order);die;
                    // Agrega credenciales
                    MercadoPago\SDK::setAccessToken('APP_USR-8616175425873682-111423-95900a228fd47acec1920d09f949310e-1548795509');
        
                    // Crea un objeto de preferencia
                    $preference = new MercadoPago\Preference();
                    $url_base = $request->getScheme() . '://' . $request->getHttpHost() . $request->getBasePath();
                    $preference->back_urls = array(
                        "success" => $url_base . $this->generateUrl("mercado_pago_request"),
                        "failure" => $url_base . $this->generateUrl("mercado_pago_failure"),
                        "pending" => $url_base . $this->generateUrl("mercado_pago_failure")
                    );
                    ...
                    $preference->external_reference = $form->getData()['orderReference'];
                    $preference->binary_mode = true;
                    //$expiresAt = new \DateTime;
                    //$preference->date_of_expiration = $expiresAt->add(new \DateInterval('PT' . $services->getSetting("checkout_timeleft") . 'S'));
            
                    // Opcional para setear las url del webhook
                    $preference->notification_url = $url_base . $this->generateUrl("mercado_pago_request");
            
                    // Crea un ítem en la preferencia
                    $item = new MercadoPago\Item();
                    $item->title = 'Entrada al Arca de Noe';
                    $item->id = $form->getData()['orderReference'];
                    $item->description = $translator->trans("Payment of tickets purchased on %website_name%", array('%website_name%' => $services->getSetting("website_name")));
                    $item->quantity = 1;
                    $item->picture_url = "https://ticket.gouservis.com/uploads/events/654436bd55c2b041623965.jpg";
                    $item->unit_price = ($orderTotalAmount/100);
                    ...
                    $payer->id = $this->getUser()->getId();
                    if ($form->getData()['firstname'] != null && $form->getData()['firstname'] != "") {
                        $payer->name = $form->getData()['firstname'];
                    }
                    if ($form->getData()['lastname'] != null && $form->getData()['lastname'] != "") {
                        $payer->surname = $form->getData()['lastname'];
                    }
                    
                    $payer->email = $form->getData()['email'];
                    $payer->date_created = "2023-11-16T12:58:41.425-04:00";
                    $payer->phone = array(
                        "area_code" => "54",
                        "number" => "949 128 866"
                    );
                    $payer->address = array(
                        "zip_code" => $form->getData()['postalcode'],
                        "street_name" => $form->getData()['street'],
                        "street_number" => ""
                    );
                    ...
                    $payoutMethod = $services->getPaymentGateways(array("organizer" => $this->getUser()->getSlug(), "slug" => $order->getPaymentGateway()->getGatewayName()))->getQuery()->getOneOrNullResult();
                    if (!$payoutMethod) {
    ...
    public function details($reference, Request $request, TranslatorInterface $translator, AppServices $services) {
        ...
        $status = null;
        if ($order->getStatus() == 1) {
            if ($order->getPaymentGateway()->getGatewayName() == "mercadopago_checkout"){
