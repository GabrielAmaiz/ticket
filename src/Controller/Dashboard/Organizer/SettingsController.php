...
class SettingsController extends Controller {
    ...
    public function payoutAddEdit(Request $request, AppServices $services, TranslatorInterface $translator, $slug = null) {
        ...
        if (!$slug) {
            ...
            if ($factoryName != "paypal_rest" && $factoryName != "stripe_checkout" && $factoryName != "mercadopago_checkout") {
            ...
            if (($factoryName == "paypal_rest" && $services->getSetting("organizer_payout_paypal_enabled") == "no") 
            || ($factoryName == "stripe_checkout" && $services->getSetting("organizer_payout_stripe_enabled") == "no")
            || ($factoryName == "mercadopago_checkout" && $services->getSetting("organizer_payout_mercadopago_enabled") == "no")) {
        ...
        if ($form->isSubmitted()) {
            ...
            if ($form->isValid()) {
                if (!$slug) {
                    ...
                    } else if ($factoryName == "mercadopago_checkout") {
                        $paymentgateway->setGatewayName("Mercado Pago");
                        $paymentgateway->setName("Mercado Pago");
                    }
                }
